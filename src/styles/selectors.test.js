...
describe('stylesheet selector groups', () => {
  ...
  it('keeps the known Graphite symptom selectors as comma groups', () => {
    ...
    expect(css).toMatch(/\.scout-layout__conditions \.scout-controls__intro\s*\{[\s\S]*?grid-template-columns: minmax\(220px, \.7fr\) 1\.3fr;/);
    expect(css).toMatch(/\.scout-layout__conditions \.scout-conditions\s*\{[\s\S]*?grid-template-columns: repeat\(3, minmax\(0, 1fr\)\);/);
